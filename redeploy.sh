...
Options:
  ...
  --reset-db       stop containers and DELETE MySQL volume (all data lost)
...
if $RESET_DB; then
  echo ""
  echo "WARNING: --reset-db will permanently delete all MySQL data in volume zreq_data."
...
echo "==> Waiting for API (migrate deploy runs on startup)..."
for _ in $(seq 1 30); do
  if docker compose ps api --format '{{.State}}' 2>/dev/null | grep -q running; then
    if docker compose logs api 2>/dev/null | grep -qE 'Nest application successfully started|Listening on'; then
      break
    fi
  fi
  sleep 2
...
if docker compose ps api --format '{{.State}}' 2>/dev/null | grep -q running; then
  echo ""
  if $RESET_DB; then
    echo "Reset complete. Fresh DB + migrate deploy done."
  else
